...
void Simulation::updatePositions(int w, int h, ITask *task)
{
    int i, j;
    float d;
    wallBounce(w, h);
        ...
        for(i=j+1; i<nBalls; i++)
            if(isInterractionCandidate(balls+j, balls+i))
            {
                ...
                if(d<=r)
                {
                    ...
                    if(ballBounce(balls+j,balls+i,r))
                        task->On();
                    keepTotalMomentum();
                }
            }
...
bool Simulation::wallBounce(int w, int h)
{
    int i;
    bool result = false;
    for(i=0; i<nBalls; i++)
...
bool Simulation::ballBounce(Ball *b1, Ball *b2, double r)
{
    int i;
    bool result = false;
